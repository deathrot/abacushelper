            ...
            var result = new Logic.Models.DBSaveResult();
             ...
             if (saveRequest != null && !saveRequest.IsEmpty)
             {
                ...
                if (saveRequest.entitesToInsert != null && saveRequest.entitesToInsert.Length > 0)
                {
                    ...
                    {
                        var insertSave = await DB.DBUtility.Insert<DBModels.QuestionEntity>(connectionUtility, itemsToInsert);

                        result.TotalRecordInserted += insertSave.TotalRecordInserted;

                        foreach (var k in insertSave.InsertKeyMap)
                        {
                            result.InsertKeyMap.Add(k.Key, k.Value);
                        }
                    }
                }
            }
