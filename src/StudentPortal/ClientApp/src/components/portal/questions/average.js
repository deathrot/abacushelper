...
const calculateAnswer = (data) => {
    if (!_.isUndefined(data) && !_.isUndefined(data.numbers) && data.numbers.length > 0) {
        ...
        return Math.round(count / data.numbers.length);
    }

    return null;
}

const Average = ({ onQuestionAnswered, data }) => {
    ...
    return (
        <div class="card">
            <div class="card-body">
                <p class="card-text">
                    <div class="card-title"><strong>Average Question:</strong></div>
                    <ul>
                        <li>Enter the average of the numbers below in the answer box below:</li>
                    </ul>
                </p>
            </div>
            {data && data.numbers &&
